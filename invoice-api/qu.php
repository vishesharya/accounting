...
$custom_params = [];
// *** FIX APPLIED HERE: Removed the check for '&' ***
// This ensures the custom '\' (backslash) parsing is always used if '\' is present, 
// allowing '&' inside parameter values.
...
function get_param_value($key) {
    ...
    if (isset($custom_params[$key])) {
        ...
        return urldecode(trim($custom_params[$key])); 
    }
    // Added trim for robustness
    return isset($_GET[$key]) ? urldecode(trim($_GET[$key])) : '';
}

// *** UPDATED: Document Type set to Quotation format ***
$document_type = 'QUOTATION'; 
$document_type = ucwords(strtolower($document_type)); // Sets it to "Quotation"
// ********************************************************

$document_prefix = 'QT'; // Assuming prefix for quotation is QT or similar
...
$shipping_address= get_param_value('Final_Shipping_Address'); // Used as shipping/delivery address
...
$po_number         = get_param_value('PO_Number'); // Assuming PO number might be used as a customer reference
...
<tr>
<td>
<div class="info-title">Quotation For (Customer)</div>
<div class="info-content">
<strong>'.$customer_name.'</strong><br>
'.$billing_address.'<br>
'.((!empty($contact_person) || !empty($customer_phone)) 
    ? '<span class="label">Contact:</span> '.$contact_person.( !empty($contact_person) && !empty($customer_phone) ? ' | ' : '' ).$customer_phone.'<br>' 
    : '' ).'
...
'.(!empty($po_number) ? '<span class="label">Customer Ref:</span> <strong>'.$po_number.'</strong><br>' : '').'
<span class="label">Issue Date:</span> <strong>'.$create_date.'</strong><br>
<span class="label">Payment Terms:</span> '.$payment_terms.'<br>
<span class="label">Validity:</span> <strong>'.$validity.'</strong>
</div>
</td>
...
<th class="col-lead center">Lead Time</th>
</tr>
</thead>
<tbody>';
...
// *** UPDATED: Disclaimer for Quotation ***
$html .= 'This is a system-generated '.$document_type.' and is subject to change. All prices are valid until the specified date and non-binding until accepted.';
$html .= '</div>';
