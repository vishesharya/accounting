...
function get_param_value($key) {
    ...
    if (isset($custom_params[$key])) {
        // Applying trim() to ensure no accidental whitespace affects decoding
...
// Format PROFORMA INVOICE to Proforma Invoice (UCWORDS)
$document_type = ucwords(strtolower($document_type));
...
if (!empty($submitted_by_phone)) {
    // UPDATED: Added '+' sign before the phone number
...
<tr>
<td>
<div class="info-title">Invoice To (Customer)</div>
...
'.(!empty($po_number) ? '<span class="label">PO Number:</span> <strong>'.$po_number.'</strong><br>' : '').'
...
$html .= 'This is a system-generated '.$document_type.' and is subject to change. Please confirm before making payment.';
$html .= '</div>';
...
// File name format: Document_DOCument_Number.pdf (as per previous request)
